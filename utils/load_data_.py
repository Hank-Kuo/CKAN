...
def load_data(args):
    ...
    n_entity, n_relation, kg, total_e = load_kg(args, kg_file)
    max_user_history_item, ripple_set = get_ripple_set(args, kg, total_e, user_history_dict)
...
def load_kg(args, kg_file):
    ...
    total_e = set.union(set(kg_np[:, 0]), set(kg_np[:, 2]))

    kg = construct_kg(kg_np)

    return n_entity, n_relation, kg, total_e
...
def get_ripple_set(args, kg, total_entity, user_history_dict):
    ...
    for user in tqdm(user_history_dict):
        for h in range(args.n_hop):
            ...
            for entity in tails_of_last_hop:
                triplets = kg[entity] 
                func = lambda x: [x[0] for x in x]
                negative_triplets = list(total_entity - set(func(triplets)) - set([entity]))
                ...
                for tail_and_relation in triplets:
                    ...
                    memories_neg_t.append(random.choice(negative_triplets))


            # sampling
            if len(memories_h) == 0:
                ripple_set[user].append([[0]*max_user_history_item, [0]*max_user_history_item,[0]*max_user_history_item,[0]*max_user_history_item])
            else:    
                # padding sampling
                ...
                memories_neg_t = [memories_neg_t[i] for i in indices] + [0]*l

                ripple_set[user].append([memories_h, memories_r, memories_t, memories_neg_t])
